...
const fetchTalentCategoryData = createAsyncThunk(
    // action명 : slice의 name/action명 붙여주기(extraReducers와 명칭 통일 필요)
    'talentCategory/fetchTalentCategoryData',
    // 실행할 비동기 함수
    async () => {
...
const talentCategorySlice = createSlice({
    name: 'talentCategory',
    initialState: talentCategoryInitialState,
    reducers: {
        // setCategory라는 type의 reducer를 호출하면,
        //  state(상태관리할 변수값)과 action(호출된 type +parameter로 전달된값(payload로 전달됨))이 전달되는데
        //  이 값을 아래와 같이 바꿔라
        setCategory(state, action) {
            state.talentCategories = action.payload;
        }
    },
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchTalentCategoryData.fulfilled, (state, action) => {
                state.talentCategories = action.payload;
                state.status = 'succeeded'; // fetch 성공
                state.initialized = true; // fetch 완료로 상태 업데이트
            })
            .addCase(fetchTalentCategoryData.pending, (state, action) => {
                state.status = 'loading';
            })
            .addCase(fetchTalentCategoryData.rejected, (state, action) => {
                state.status = 'failed';
